...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("MergeTreeTemporalReductionDecoding");

  // ---------------------------------------------------------------------------
  // Initialize ttkMergeTreeTemporalReductionDecoding module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto mergeTreeTemporalReductionDecoding
    = vtkSmartPointer<ttkMergeTreeTemporalReductionDecoding>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // mergeTreeTemporalReductionDecoding->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkMergeTreeTemporalReductionDecoding filter
      mergeTreeTemporalReductionDecoding->SetInputDataObject(
        i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    mergeTreeTemporalReductionDecoding->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkMergeTreeTemporalReductionDecoding filter
  // ---------------------------------------------------------------------------
  mergeTreeTemporalReductionDecoding->Update();
  ...
  if(!outputPathPrefix.empty()) {
    for(int i = 0;
        i < mergeTreeTemporalReductionDecoding->GetNumberOfOutputPorts(); i++) {
      auto output = mergeTreeTemporalReductionDecoding->GetOutputDataObject(i);
      auto writer
