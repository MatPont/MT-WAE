...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("MergeTreeTemporalReductionEncoding");

  // ---------------------------------------------------------------------------
  // Initialize ttkMergeTreeTemporalReductionEncoding module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto mergeTreeTemporalReductionEncoding
    = vtkSmartPointer<ttkMergeTreeTemporalReductionEncoding>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // mergeTreeTemporalReductionEncoding->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkMergeTreeTemporalReductionEncoding filter
      mergeTreeTemporalReductionEncoding->SetInputDataObject(
        i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    mergeTreeTemporalReductionEncoding->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkMergeTreeTemporalReductionEncoding filter
  // ---------------------------------------------------------------------------
  mergeTreeTemporalReductionEncoding->Update();
  ...
  if(!outputPathPrefix.empty()) {
    for(int i = 0;
        i < mergeTreeTemporalReductionEncoding->GetNumberOfOutputPorts(); i++) {
      auto output = mergeTreeTemporalReductionEncoding->GetOutputDataObject(i);
      auto writer
