...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("MergeTreePrincipalGeodesicsDecoding");

  // ---------------------------------------------------------------------------
  // Initialize ttkMergeTreePrincipalGeodesicsDecoding module (adjust
  // parameters)
  // ---------------------------------------------------------------------------
  auto mergeTreePrincipalGeodesicsDecoding
    = vtkSmartPointer<ttkMergeTreePrincipalGeodesicsDecoding>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // mergeTreePrincipalGeodesicsDecoding->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkMergeTreePrincipalGeodesicsDecoding filter
      mergeTreePrincipalGeodesicsDecoding->SetInputDataObject(
        i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    mergeTreePrincipalGeodesicsDecoding->SetInputArrayToProcess(
      i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkMergeTreePrincipalGeodesicsDecoding filter
  // ---------------------------------------------------------------------------
  mergeTreePrincipalGeodesicsDecoding->Update();
  ...
  if(!outputPathPrefix.empty()) {
    for(int i = 0;
        i < mergeTreePrincipalGeodesicsDecoding->GetNumberOfOutputPorts();
        i++) {
      auto output = mergeTreePrincipalGeodesicsDecoding->GetOutputDataObject(i);
      auto writer
