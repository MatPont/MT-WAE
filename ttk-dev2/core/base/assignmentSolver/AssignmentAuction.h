...
namespace ttk {
  ...
  dataType getLowerBoundCost(std::vector<std::vector<dataType>> &costMatrix,
                             ...
                             std::stringstream &ss) {
    TTK_FORCE_USE(rowSize);
    TTK_FORCE_USE(colSize);
    TTK_FORCE_USE(balancedAssignment);
    ...
    std::vector<unsigned int> minColIndex(minCol.size()),
      minRowIndex(minRow.size());
    ...
    std::stringstream ssCol, ssRow;
    for(unsigned int i = 0; i < minCol.size(); ++i) {
      minColCost += minCol[i];
      ssCol << minColIndex[i] << " _ " << i << " _ " << minCol[i] << std::endl;
    }
    for(unsigned int i = 0; i < minRow.size(); ++i) {
      minRowCost += minRow[i];
      ssRow << i << " _ " << minRowIndex[i] << " _ " << minRow[i] << std::endl;
    }
    dataType minCost = std::max(minColCost, minRowCost);
    if((rowSize + colSize) / 2 <= 100) {
      if(minColCost > minRowCost)
        ss << ssCol.str();
      else
        ss << ssRow.str();
    }
  ...
  int AssignmentAuction<dataType>::run(std::vector<MatchingType> &matchings) {
    ...
    initEpsilon();
    bestCost = std::numeric_limits<dataType>::max();
    ...
    minCost = getLowerBoundCost(this->costMatrix, this->rowSize, this->colSize,
                                this->balancedAssignment, ss);
    ss << "minCost  = " << minCost << std::endl;
    ...
    ss << "bestCost    = " << bestCost << " (iter " << bestIter << " / " << iter
       << ")" << std::endl;
    ss << "need weight = " << (bestCost / minCost) << std::endl;

    if((this->rowSize + this->colSize) / 2 <= 100)
      for(unsigned int bidderId = 0; bidderId < bidderAssignments.size();
          ++bidderId) {
        int i = bidderId;
        int j = bidderAssignments[bidderId];
        if(not(i >= this->rowSize - 1 and j >= this->colSize - 1))
          ss << i << " _ " << j << " _ " << this->costMatrix[i][j] << std::endl;
      }
  ...
  dataType AssignmentAuction<dataType>::getRelativePrecision(
    ...
    if(d < 1e-6 or d <= (minCost * minCostWeight)) {
      return 0;
    }
