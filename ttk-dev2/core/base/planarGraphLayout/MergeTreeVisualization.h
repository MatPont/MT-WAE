...
namespace ttk {
  ...
  protected:
    ...
    double nonImportantBranchSpacing_ = 1.;
  ...
  public:
    ...
    void setNonImportantBranchSpacing(double d) {
      nonImportantBranchSpacing_ = d;
    }
      ...
      while(!queue.empty()) {
        ...
        if(tree->isBranchOrigin(nodeOrigin)) {
          ...
          for(size_t i = 0; i < nodeBranchingVector.size(); ++i) {
            ...
            bool isImportantNodeBranching = tree->isImportantPair<dataType>(
              nodeBranchingI, importantPairs_,
              excludeImportantPairsLowerValues_,
              excludeImportantPairsHigherValues_);
            float shiftY
              = nodeBranchingIPers
                * (isImportantNodeBranching ? branchSpacing_
                                            : nonImportantBranchSpacing_);
